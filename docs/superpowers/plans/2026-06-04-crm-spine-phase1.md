...
## Execution record (2026-06-04) — built & verified

**Status: Phase 1 COMPLETE on `feat/crm-spine`.** Backend live on Supabase `bjbvqvzbzczjbatgmccb`.

**Deviations from plan (intentional, decided during execution):**
1. **Resolver fuzzy-match gated to keyless rows.** Original cascade ran fuzzy on every unlinked row. Problem: every `outreach_prospect`/`lead` has a unique `linkedin_url`; two distinct people with similar names would have collapsed into a stuck `pending` link, leaving the second person with no active contact. Fix: fuzzy step now runs **only when `linkedin_url IS NULL`** (migration `crm_resolver_fuzzy_gate`). Keyed rows go exact-match-or-create-new. Fuzzy is reserved for the keyless Phase-2 sources (transcripts/proposals).
2. **Schedule via `pg_cron`, not n8n.** Chose `cron.schedule('crm-resolve-contacts','*/30 * * * *', ...)` (jobid 6, active) over a thin n8n workflow — fewer moving parts, no credential wiring, avoids the n8n Code-node binary/loop traps that have caused prior incidents. The dashboard "Resolve now" button covers on-demand. WhatsApp error-notify deferred to Phase 2.

**Live verification results:**
- `leads` is currently **empty (0 rows)**; 1,598 `outreach_prospects`. First resolve → `{created:1598, linked:0, pending:0}`; re-run → all-zero (idempotent). 1,598 contacts ↔ 1,598 active links, 0 multi-link (no intra-source dupes). The lead↔prospect merge win is dormant until `leads` populates.
- `get_contact_360` verified on "Deana Stone" — returns contact + links + 3-event timeline incl. real DM text, newest-first.
- Playwright (`local-dash` profile, dev server :5191): CRM renders as first/default Reach sub-tab; list shows 500 real contacts with ICP; clicking a contact opens the inline record with stage selector, CRM fields, sources chip, manual Attach, and timeline. Screenshots: `/tmp/pw-out/crm-list.png`, `/tmp/pw-out/crm-record.png`.

**Known cosmetic follow-ups (→ Phase 2 UI pass):** SOURCES chip low-contrast; non-DM timeline events render raw JSON; list filters (ICP band / needs-review / overdue) still stubbed.

**Migrations applied (remote):** `crm_spine`, `crm_resolver`, `crm_resolver_fuzzy_gate`, `crm_360`, plus `pg_cron` job.
