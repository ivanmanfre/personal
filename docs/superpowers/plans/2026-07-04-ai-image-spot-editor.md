...
### Task 12: `/client` board mount (Phase-2 boundary — decision-gated)

**Files:**
- Modify: `components/ClientBoardPage.tsx` (image card `Thumb` region `:214-221`, `:659`, `:831`)

**DECISION REQUIRED before building:** the `/client` board is currently a **demo/read-only** artifact populated by the `bootstrap-client-board` skill; its images live on `client_boards.board` items as `media_url`/`cover_url`, NOT on `carousel_drafts`. Two options — confirm with Ivan which:
- (a) **Ephemeral demo edits** — let a prospect edit the demo image in-session (impressive in a live walkthrough) but don't persist to `client_boards`. Cheapest; the wow-factor without a write path.
- (b) **Persisted client edits** — edits write back to `client_boards.board` media_url + a version row. Needs a board-scoped commit path distinct from `commitImageEdit` (which targets `carousel_drafts`).

- [ ] **Step 1: Get the (a)/(b) decision from Ivan.** Do NOT build until answered.
- [ ] **Step 2 (if a):** add an "✨ Edit" affordance on `Thumb` that opens `ImageEditorModal` with a local-only `onCommitted` that swaps the rendered `src` in component state and skips DB commit. Add the per-board cost cap (Global Constraints).
- [ ] **Step 3 (if b):** add `commitClientBoardImage()` to a client-board actions module writing `client_boards.board` + `image_edit_versions` (with a `board` op tag), and wire `onCommitted` to it.
- [ ] **Step 4:** Census → exit 0. Self-test the chosen path. Commit.
...
- **Spec coverage:** click-spot region-lock (Tasks 4,8) ✓; Erase/Replace/Refine (Task 5,9) ✓; proposal loop Keep/Try/Back (Tasks 1,7,10) ✓ — the Fable safety lever; preset chips (Tasks 2,9) ✓; global command bar / free-comment no-click (Tasks 2,5,9,10) ✓ — Ivan's addition; latency honesty (Task 10) ✓; cost cap (Tasks 1,10,12) ✓; save-keeps-original + undo (Tasks 1,6,10) ✓; mount everywhere (Tasks 11,12) ✓ with `/client` gated on a decision. Real-layer text + drag + auto-flag correctly deferred with reasons.
