...
import { asset, readerFor, type JourneyFixture } from './model';
...
export function ResourceChapter({ fixture }: {fixture: JourneyFixture}) {
  ...
  return <>
    ...
    <Reveal className="lead-card" aria-label="Lead record"><div className="lead-card-head"><span className="sample-stamp">New lead</span><span className="lead-time">just now</span></div><div className="demo-person"><span className="demo-avatar" aria-hidden="true">{reader.initials}</span><div><b>{reader.name}</b><span>{reader.headline}</span></div></div>
      <dl><div><dt>Came in through</dt><dd data-testid="record-context">LinkedIn → {lm.title}</dd></div><div><dt>Post they read first</dt><dd>{fixture.samples.posts?.[0]?.hook}</dd></div><div><dt>Next</dt><dd>Day 1 message goes out. Day {fixture.samples.follow_ups?.[1]?.day} and day {fixture.samples.follow_ups?.[2]?.day} follow if {reader.first} stays quiet.</dd></div></dl></Reveal>
  </>;
}
