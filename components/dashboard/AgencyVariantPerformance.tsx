...
// Tag short-labels are stable identifiers (used in n8n round-robin code) — safe to hardcode.
// Copy is NOT hardcoded — extracted dynamically from sample_text per row so edits in the
// n8n Send DM node flow through automatically on the next send.
...
// Touch-1 DM format (always):
//   Hi {first},\n\n{BODY}\n\n{URL}\n\n{CLOSER}\n\nIván
// Splitting on \n\n yields [salutation, body, url, closer, signoff].
function extractCopy(sampleText: string | null | undefined, axis: 'body' | 'closer'): string | null {
  if (!sampleText) return null;
  const parts = sampleText.split('\n\n').map((s) => s.trim()).filter(Boolean);
  if (parts.length < 4) return null;
  return axis === 'body' ? parts[1] : parts[parts.length - 2];
}
...
function VariantRow({
  ...
  const copy = extractCopy(row?.sample_text, axis);
  return (
    <div
    ...
    >
      ...
      <p className="text-[10px] text-zinc-500 italic truncate mb-1">
        {copy ? <>&ldquo;{copy}&rdquo;</> : <span className="text-zinc-600">copy will appear after first send</span>}
      </p>
