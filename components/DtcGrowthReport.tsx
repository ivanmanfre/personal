...
// Receipt price formatting: whole prices stay whole, fractional prices get two decimals, so
// the receipt reads back the catalog value the way the catalog states it.
...
function ProfitGapSpread({
  ...
  const cacSub = adsEmpty && cac === 0
    ? 'no active ads found, so the seed carries none'
    : 'drag the slider to what you pay today';
  ...
  const ledgerRows: Array<{ key: string; name: string; sub: string; v: number; swatch: string; dashed?: boolean; gold?: boolean }> = [
    ...
    { key: 'cogs', name: 'Cost of goods', sub: `${cogsPct}% of what is kept`, v: cogsSeg, swatch: HATCH_CSS },
    ...
    { key: 'contribution', name: 'Contribution per order', sub: 'what is left before acquisition', v: contribution, swatch: accent, gold: true },
      ...
      <div className="mx-auto w-full max-w-[1180px] px-6 sm:px-8 py-20 sm:py-28">
        ...
        <div className="mt-16">
          ...
          <p className="mt-4 text-[0.84rem] leading-relaxed" style={{ color: surface, opacity: 0.5, maxWidth: '66ch' }}>
            {seeded
              ? 'Segment widths are proportional to the ledger below. AOV is seeded from your public median price. Your real cost lines replace these on the call.'
              : 'Segment widths are proportional to the ledger below. Type your real numbers into the inputs.'}
          </p>
...
export function DtcGrowthReport({ report, scan, companyName }: { report: ReportJson; scan: Scan; companyName: string }) {
  ...
  if (shopData) {
    ...
    if (
      band &&
      (cited(haysShopify, fmtPrice(band.min).slice(1)) ||
        cited(haysShopify, fmtPrice(band.max).slice(1)) ||
    ...
    if (band && band.median != null && (pgSeeded || cited(haysShopify, fmtPrice(band.median).slice(1)))) {
    ...
    // Catalog size heads the group when any other catalog line stands on it, or when a
    // finding cites the count itself.
    if (typeof shopData.catalog_size === 'number' && (rest.length > 0 || cited(haysShopify, String(shopData.catalog_size)))) {
  ...
  // The receipt's single gold moment: the first line carrying the lead finding's signal.
  const flagSignal = findings[0]?.signal || null;
  const flagLine = flagSignal
    ? receiptGroups.flatMap((g) => g.lines).find((l) => l.signal === flagSignal) || null
    : null;
        ...
        >
          <div className="mx-auto w-full max-w-[1180px] px-6 sm:px-8 py-14 sm:py-16">
            ...
            >
              ...
              <div
                className="px-[22px] py-3 text-[0.62rem] font-bold uppercase tracking-[0.14em] leading-relaxed"
                style={{ borderTop: '1px dashed #dfe3e7', fontFamily: headingFont, color: ink, opacity: 0.45 }}
              >
                {scanDate ? `Read ${scanDate} from public pages. No account access, no data you gave us. ` : ''}
