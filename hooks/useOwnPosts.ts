  ...
  const fetch = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: qErr } = await supabase
        ...
        .select('id, post_text, post_type, num_likes, num_comments, num_shares, num_impressions, posted_at, linkedin_url, topic_category, hook_pattern, pillar, metrics_updated_at, post_kind, lead_magnet_slug, named_leads, named_leads_30d, capture_rate_pct, also_promoted_by')
        .gte('posted_at', since)
