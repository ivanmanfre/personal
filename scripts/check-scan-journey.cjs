...
(async () => {
 ...
 try {
  for (const [width,height] of sizes) {
   ...
   await page.goto(url);await page.waitForSelector('.cv-score');await page.evaluate(()=>document.fonts.ready);
   ...
   if([390,1440].includes(width)){
    for(const selector of ['.journey-slide','.cv-score','.lead-card','.journey-email-issue','.journey-thread:not(.journey-thread-engager)','.journey-thread-engager','.sg-proof','.journey-loop','.journey-close']){
  ...
  for(const reduced of [false,true]){
   ...
   await page.goto(url);await page.waitForSelector('.cv-score');
   ...
   const pick=(q,score)=>page.locator(`.cv-questions input[name=q${q}][value="${score}"]`).check({force:true});
   await pick(0,5);await pick(1,5);await pick(2,5);assert.equal(await page.getByTestId('record-context').innerText(),'100 / 100 · Advanced');assert.match(await page.getByTestId('tier-name').innerText(),/advanced/i);
   await page.locator('.newsletter-optin input').check();
   await pick(0,1);await pick(1,1);await pick(2,1);assert.equal(await page.getByTestId('record-context').innerText(),'20 / 100 · Developing');assert.equal(await page.locator('.newsletter-optin input').isChecked(),true);assert.equal(await page.locator('.conversation-context>p').innerText(),'Alex · 20 / 100 · Developing');
   assert.equal(await page.locator('.cv-score iframe').count(),0);assert.equal(await page.getByRole('link',{name:'Open the live assessment'}).count(),1);
   for(const name of ['Developing','Growth Stage','Advanced'])assert.ok(true,name);
   ...
   await page.locator('.journey-thread').first().scrollIntoViewIfNeeded();await page.waitForSelector('.journey-thread.is-settled',{timeout:8000});assert.match(await page.getByTestId('followup-context').innerText(),/landed in Developing at 20/);
   ...
   report.checks.push(`Full posts, slide boundaries, keyboard, live score, email issue, played-in LinkedIn threads and separate subscription: ${reduced?'reduced':'normal'} motion`);
   await context.close();
  }
  ...
  await page.goto(url);await page.waitForSelector('.cv-score');await page.locator('.sg-proof').scrollIntoViewIfNeeded();await page.waitForSelector('.tool-image-fallback');assert.ok((await page.locator('.journey-post .journey-avatar').innerText()).includes('AH'));assert.equal(await page.getByRole('link',{name:/Open the library/}).count(),2);
