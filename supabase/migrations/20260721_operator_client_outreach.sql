-- Client Ops — per-client OUTREACH read surface (W2, RISE DTC round 2).
--
-- ONE gated SECURITY DEFINER RPC that returns everything the Client Ops panel
-- needs to render two views for a client's outreach:
--   1. Simple list  — every prospect across the client's outreach_campaigns,
--                      lane-grouped, with the ACTUAL lane sequence copy (the
--                      ratified board.outreach.sequences package), per-prospect
--                      generated copy when it exists, and gate flags
--                      (enrichment_data.name_gate = blocked until the client OKs
--                      an anchor name).
--   2. Priority      — leads that have been messaged with no reply yet, plus the
--                      higher-priority "they replied, you owe them" bucket. Both
--                      read the SAME columns the Conversation Monitor already
--                      stamps (prospect.last_dm_sent_at / last_reply_at /
--                      reply_count / needs_manual_reply) and the inbound rows in
--                      outreach_messages. NO new polling infra.
--
-- READ ONLY. Nothing here sends. Campaigns are looked up by client_id, so this
-- works for any productized client; RISE DTC is simply the first.
--
-- Gate: identical to every other operator_* RPC (operator_gate_ok on the shared
-- operator_panel_gate_hash). Authenticated-only; anon revoked.
...
begin
  ...
  -- Ratified sequence package + lane meta live on the client's board JSON.
  ...
  -- Are any campaigns armed? (Guardrail truth for the surface's NOT-ARMED banner.)
  select bool_or(coalesce(c.is_active, false))
    into v_armed
  from outreach_campaigns c
  where c.client_id = p_client_id and coalesce(c.archived, false) = false;

  -- Campaigns for this client with a derived lane key + honest counts.
  -- lane_key maps a campaign onto its sequence in the ratified package. Network
  -- Activation has no ratified sequence (lane retired) → null, surfaced honestly.
  ...
  -- Every prospect across those campaigns (the simple list). Carries gate flags,
  -- any per-prospect generated copy, and the real message trail (sent + replies)
  -- from outreach_messages so the priority view has evidence the moment sends go
  -- live. Ordered strongest-ICP first.
...
comment on function public.operator_client_outreach(text, text) is
  'Client Ops read surface: per-client outreach list + priority (waiting-on-response) view. Gated (operator_gate_ok), read-only, reads the reply infra the Conversation Monitor already writes. W2 2026-07-21.';
