-- Prepared separately from Run07: evaluate repeated canonical resolution once.
CREATE OR REPLACE VIEW public.audn_identity_resolution_v WITH (security_invoker=true) AS
 WITH identity_cache AS MATERIALIZED (select * from public.audn_canonical_post_identity_v), candidates AS MATERIALIZED (
         SELECT s.id AS raw_snapshot_id,
           ...
           FROM ((audn_post_metric_snapshots s
             LEFT JOIN identity_cache exact_i ON (((exact_i.client_id = s.client_id) AND (exact_i.identity_post_social_id = s.post_social_id))))
...
CREATE OR REPLACE VIEW public.audn_post_owner_resolution_v WITH (security_invoker=true) AS
 WITH raw_ids AS MATERIALIZED (
        ...
        ), raw_keys AS MATERIALIZED (select post_social_id,audn_urn_kind(post_social_id) raw_kind,audn_urn_digits(post_social_id) raw_digits from raw_ids), identity_cache AS MATERIALIZED (select * from public.audn_canonical_post_identity_v), candidates AS (
         SELECT r_1.post_social_id,
            i.client_id,
            i.canonical_post_id
           FROM (raw_keys r_1
             JOIN identity_cache i ON (((i.identity_status = 'resolved'::text) AND ((i.canonical_post_id = r_1.post_social_id) OR ((r_1.raw_kind IS NULL) AND (i.activity_digits = r_1.raw_digits))))))
        ), g AS (
